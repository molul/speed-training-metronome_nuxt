...
const getRowBgClass = (bpmVarName: string, rowIndex: number) => {
  ...
  if (bpmVarName === "startBpm" && bpm === startBpm) return "bg-green-400";
  if (bpmVarName === "peakBpm" && bpm === peakBpm) return "bg-red-300";
  if (bpmVarName === "endBpm" && bpm === endBpm) return "bg-yellow-400";
...
<template>
  <div class="flex flex-col select-none z-10 -translate-x-[1px]">
    ...
    >
      <div
        class="size-full absolute top-0 left-0 flex gap-0 rounded-l-xs overflow-hidden"
      >
        <div
      ...
      <span
        :class="[
          'z-30 -translate-x-[1px] translate-y-[1px] font-bold font-monocode leading-none',
          {
            'text-zinc-900 dark:text-zinc-900': [
              store.config.startBpm,
              store.config.peakBpm,
              store.config.endBpm,
            ].includes(store.rowToBpm(r) + 5),
          },
        ]"
      >
        {{ store.rowToBpm(r) + 5 }}
